    ...
    def __init__(self, numImages):
        ...
        for i in range(0, self.numMarker):
            ...
            path = str("/home/fer/Desktop/catkin_ws/src/AMCL_Hybrid/detector/markers_alma"+marker+".png")  #PENDING modificar para tomar path desde launch file
                                                #PARECE que en este path estn guardadas las marcas y este script compara topic vs. marcas guardadas
            print(path)
        ...
        #self.image_sub = rospy.Subscriber("/camera/RGB/image_raw", Image, self.callback) #PENDING de aqui toma la info.   
        self.image_sub = rospy.Subscriber("/camera/RGB/Image", Image, self.callback) #PENDING de aqui toma la info.
...
def main(args):
    ...
    image_converter(4)  #num imgs con las marcas
    try:
        rospy.spin()
