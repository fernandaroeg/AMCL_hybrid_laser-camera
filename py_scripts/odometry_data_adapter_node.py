...
#Odometry adapter node converts 2D pose (x,y, theta) messages to Odometry msg
...
var_x = 0.1 # covariance in x, value taken from turtlebot sim in gazebo
var_y = 0.1# covariance in y, value taken from turtlebot sim in gazebo
var_theta = 0.1 #0.01 # covariance in z/yaw, value taken from turtlebot sim in gazebo
noise_range_x = 0.1 #1cm expressed in meters
noise_range_y = 0.1 #1cm expressed in meters
noise_range_theta = 0.17 #0.1 degreed expressed in radians
...
def oplus(x1, y1, theta1, x2, y2, theta2):
   ...
   #theta_oplus = (theta1 + theta2) 
  
   return x_oplus, y_oplus, theta_oplus

def add_gaussian_noise(prev_x, prev_y, prev_theta, x,y,theta, var_x, var_y, var_theta, noise_range_x, noise_range_y, noise_range_theta):
    #Compute the position increment  
    increment_x     =   x   - prev_x
    increment_y     =   y   - prev_y 
    increment_theta = theta - prev_theta
    
    #Add the gaussian noise to the current position increment
    increment_w_noise_x      = increment_x     + (np.sqrt(var_x)     * np.random.normal(0,noise_range_x))
    increment_w_noise_y      = increment_y     + (np.sqrt(var_y)     * np.random.normal(0,noise_range_y))
    increment_w_noise_theta  = increment_theta + (np.sqrt(var_theta) * np.random.normal(0,noise_range_theta))
    ...
    odom_now_noise_x, odom_now_noise_y, odom_now_noise_theta = oplus(x, y, theta, increment_w_noise_x, increment_w_noise_y, increment_w_noise_theta)
...
for i in range(0,len(poseX)):
    ...
    odom_now_noise_x, odom_now_noise_y, odom_now_noise_theta = add_gaussian_noise(prev_x, prev_y, prev_theta, current_x, current_y, current_theta, var_x, var_y, var_theta, noise_range_x, noise_range_y, noise_range_theta)
    ...
    #quaternion_angle = tf.transformations.quaternion_from_euler(0, 0, current_theta)
    #odometry_msg.pose.pose = Pose( Point(current_x, current_y, 0.0),  Quaternion(*quaternion_angle) )  #investigar q hace ese apuntador
    ...
    file.write(str(odometry_msg.pose.pose.position.x))
    file.write(' ')
    file.write(str(odometry_msg.pose.pose.position.y))
    file.write('\n')
