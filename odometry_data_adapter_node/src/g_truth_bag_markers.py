...
# def create_path(x, y, theta, t, seq):
    # path = Path()
    # path.header.seq = seq
    # path.header.stamp = t
    ...
    # pose_stamped.pose = Pose( Point(x, y, 0.0),  Quaternion(*quaternion_angle) )  
...
for i in range(0,len(poseX)):
    ...
    #Create Path msg from gtruth
    #g_truth_path = create_path(poseX[i], poseY[i], poseTheta[i], tstamp[i], i)
